# BÁO CÁO TỔNG KẾT BUỔI SEMINAR: DIVIDE AND CONQUER

## BẢNG MỤC LỤC
+ [Mô tả môn học](#mô-tả-môn-học)
+ [Thời gian, hình thức, thành phần tham dự](#thời-gian-hình-thức-thành-phần-tham-dự)
+ [Nội dung trong lúc thuyết trình](#nội-dung-trong-lúc-thuyết-trình)
+ [Phân công công viêc](#phân-công-công-việc)
	- [I. Phần báo cáo](#i-phần-báo-cáo)
	- [II. Phần bài tấp](#ii-phần-bài-tập)
+ [Tổng kết đánh giá từng thành viên](#tổng-kết-đánh-giá-từng-thành-viên)
	- [1. Tự đánh giá](#1-tự-đánh-giá)
	- [2. Nhóm biểu quyết cho đánh giá](#2-nhóm-biểu-quyết-cho-đánh-giá)
+ [Slide thuyết trình](#slide-thuyết-trình)
+ [Câu hỏi và bài tập](#câu-hỏi-và-bài-tập)
+ [Đáp án và bộ test](#đáp-án-và-bộ-test)
+ [Sửa bài tập](#sửa-bài-tập)
+ [Thống kê buổi học](#thống-kê-buổi-học)
+ [Thống kê các nhóm làm bài tập HACKERRANK](#thống-kê-các-nhóm-làm-bài-tập-hackerrank)
	-  [Thống kê điểm các nhóm tham gia chủ đề Divide and Conquer](#thống-kê-điểm-các-nhóm-tham-gia-chủ-đề-divide-and-conquer)
+ [Đánh giá về chủ đề của nhóm](#đánh-giá-về-chủ-đề-của-nhóm)

## MÔ TẢ MÔN HỌC
1. Tên môn học: Phân tích và thiết kế thuật toán
2. Mã môn học: CS112
3. Mã lớp: CS112.L21
4. Giảng viên: Thầy Nguyễn Thanh Sơn - sonnt@uit.edu.vn

## THỜI GIAN, HÌNH THỨC, THÀNH PHẦN THAM DỰ

1. Thời gian: 14h30p ngày 26/03/2021
2. Hình thức: Trực tiếp trên lớp
3. Thành phần tham dự:
	- Giảng viên lớp CS112.L21: Thầy Nguyễn Thanh Sơn
	- Sinh viên lớp CS112.L21
4. Chủ trì buổi thuyết trình: Nhóm 2
	- Lê Văn Phước - 19522054
	- Nguyễn Xuân Minh - 19521848
	- Nguyễn Đức Thắng - 19522206
	- Trầm Hữu Nguyên - 19521921
5. Thư ký:
	Nhóm 2 thay đổi luân phiên nhau ghi chép thông tin buổi thuyết trình
	
## NỘI DUNG TRONG LÚC THUYẾT TRÌNH
1. Số thứ tự chủ đề: 4
2. Tên chủ đề thuyết trình: Giới thiệu phương pháp thiết kế thuật toán: Divide and Conquer
3. Link GitHub: https://github.com/levanphuoc-dev/CS112.L21

## PHÂN CÔNG CÔNG VIỆC

### I. Phần báo cáo
**1. Chuẩn bị**

a. Lê Văn Phước:
  - Tìm hiểu sơ qua về chủ đề báo cáo, phân công công việc cho các thành viên trong nhóm.
  - Tìm hiểu về đặc điểm, dấu hiệu nhận dạng ưu nhược điểm của thuật toán.
  - Tổng hợp các phần công việc đã giao cho các bạn trong nhóm.
  - Tìm kiếm 1 bài toán về chủ đề divide and conquer để giao bài tập về nhà, chuẩn bị testcase và đáp án.

b. Nguyễn Xuân Minh
- Làm slide báo cáo.
- Tìm hiểu về độ phức tạp của thuật toán.
- Tìm kiếm 1 bài toán về chủ đề divide and conquer để giao bài tập về nhà, chuẩn bị testcase và đáp án.

c. Nguyễn Đức Thắng

- Tìm hiểu một số bài toán minh họa cho thuật toán.
- Tìm kiếm 1 bài toán về chủ đề divide and conquer để giao bài tập về nhà, chuẩn bị testcase và đáp án.

d. Trầm Hữu Nguyên

- Tìm hiều về một số bài toán bài toán minh họa.

**2. Báo cáo trên lớp**

a. Lê Văn Phước, Nguyễn Xuân Minh
  - Thông kê câu trả lời của các nhóm, ghi chú nhận xét của thầy, tổng hợp các câu hỏi thắc mắc của các nhóm chưa giải đáp được.
  - Giải bài tập về nhà.

b. Trầm Hữu Nguyên
  - Thuyết trình về phần lý thuyết của thuật toán

c. Nguyễn Đức Thắng
  - Thuyết trình về bài tập minh họa.
 
**3. Sau báo cáo**

a. Lê Văn Phước, Nguyễn Xuân Minh
  - Tổng kết, bổ sung slide thuyết trình, upload tài liệu lên Github
  - Tổng hợp, đánh giá bài tập đã cho về nhà.

### II. Phần bài tập
**1. Bài tập thực hành Wecode**
  - Làm bài tập hàng tuần chủ yếu là 3 bạn Nguyễn Xuân Minh, Lê Văn Phước, Nguyễn Đức Thắng cùng đưa ra ý kiến và giải bài tập.
  - Phần báo cáo ipynd chủ yểu được làm bởi Lê Văn Phước và Nguyễn Xuân Minh.

**2. Bài tập về nhà của các nhóm**
  - Chia luân phiên các bạn trong nhóm làm hàng tuần.

## TỔNG KẾT ĐÁNH GIÁ TỪNG THÀNH VIÊN
  Căn cứ vào cuộc họp Tổng kết môn học:
### 1. Tự Đánh Giá
a. Nguyễn Xuân Minh
-	Các đóng góp cho nhóm:
	  + Làm slide thuyết trình
	  + Tham gia viết các bài báo cáo lên github.
	  + Tham gia giải hầu hếu các bài tập wecode thầy giao.
	  + Tham gia tạo bài tập hackerrank ( bài tổng chuỗi con ngắn nhất).
	  + Tham gia giải các bài tập tự luyện các nhóm trên hackerrank.
	  + Tham gia thay mặt nhóm chơi kahoot của các nhóm khác.
	  + Tham gia phát biểu xây dựng tiết học lấy điểm cộng cho nhóm.
-	Sau khi hoàn thành khóa học thì mình tự nhận xét bản thân đã cố gắng hết sức đem đóng góp xây dựng tích cực cho nhóm, hầu như các bài tập thầy giao mình đều tích cực tham gia giải, động viên các bạn trong nhóm cùng tham gia. Bên cạnh đó thì mình tự nhận xét là mình vẫn còn chưa chủ động trong việc đóng góp ý kiến, phát biểu trong các buổi học lắm.
-	Tự đánh giá: 8.5~9/10 điểm.

b. Lê Văn Phước
- Các đóng góp cho nhóm:
  + Nắm bắt môn học, phân công nhiệm vụ công việc cho các bạn.
  + Viết báo cáo các bài tập các hoạt động trên lớp lên github.
  + Cùng tham gia giải bài tập với các thành viên trong nhóm.
  + Đại diện nhóm tham gia các hoạt động của các nhóm.
- Sau khi hoàn thành khóa học thì mình tự nhận xét bản thân mình đã cố gắng hết sức với vai trò là trưởng nhóm. Bên cạnh đó mình tự nhận xét mình vẫn chưa tích cực phê bình các bạn nhác hoặc không trả  lời tin nhắn khi mình giao công việc, chưa tích cực đại diện nhóm phát biểu bài.
- Tự đánh giá: 8.5/10 điểm

c. Nguyễn Đức Thắng.
- Các đóng góp cho nhóm:
  + Giải quyết được một số bài tập wecode.
  + Tham gia chuẩn bị và thuyết trình báo cáo.
  + Tham gia giải bài tập về nhà của các nhóm.
- Sau khi hoàn thành khóa học thì mình tự nhận xét bản thân tích cực tham gia các hoạt động của nhóm và ở lớp.
- Tự đánh giá: 8/10 điểm

d. Trầm Hữu Nguyên
- Các đóng góp cho nhóm:
  + Tham gia chuẩn bị và thuyết trình báo cáo
- Sau khi hoàn thành khóa học thì mình tự nhận xét bản thân chưa tích cực tham gia hoạt động của nhóm.
- Tự đánh giá: 7/10 điểm

### 2. Nhóm biểu quyết cho đánh giá

Theo sự thống nhất của các thành viên thì:
  + Tốt: cần cả 3 người đồng ý
  + Khá: chỉ cần 2 người đồng ý
  + Trung bình: chỉ cần 1 người đồng ý

| STT | Thành viên | Phước | Minh | Thắng | Nguyên | Kết quả |
| :---: | --- | --- | --- | --- | --- | --- |
| 1 | Lê Văn Phước | - | + | + | + | Tốt |
| 2 | Nguyễn Xuân Minh | + | - | + | + | Tốt |
| 3 | Nguyễn Đức Thắng | + | + | - | + | Tốt |
| 4 | Trầm Hữu Nguyên | - | + | + | - | Khá |


## SLIDE THUYẾT TRÌNH

| STT | Thông tin file | Đường dẫn |
| :---: | --- | --- |
| 1 | PowerPoint Slide | [Slide_Divide_and_Conquer.pptx](./Slide_Divide_and_Conquer.pptx) |
| 2 | PDF Slide | [Slide_Divide_and_Conquer.pdf](./Slide_Divide_and_Conquer.pdf) |

## CÂU HỎI VÀ BÀI TẬP

| STT | Thông tin| Đường dẫn | Hạn nộp |
| :---: | --- | --- | :---: |
| 1 | Hackerrank | [Hackerrank.com](https://www.hackerrank.com/cs112-l21-n2?fbclid=IwAR2YOnubS0sTQVflAGehAsLyx_mgC-KztVcvHkcBAMAVHKdrGAoKvpQczTI) | 02/04/2021 |

## ĐÁP ÁN VÀ BỘ TEST

| STT | Tên bài | Đáp án | Bộ test | Số lượng test case |
| :---: | --- | --- | --- | :---: |
| 1 | Hàng cây | [Code python](./Homework/HANG_CAY/Solutions/main.py) | [Testcase](./Homework/HANG_CAY/Testcases/) | 2 (+ 1 HackerRank sample) |
| 2 | Tổng dãy con lớn nhất | [Code python](./Homework/TONG_DAY_CON_LON_NHAT/Solutions/main.py) | [Testcase](./Homework/TONG_DAY_CON_LON_NHAT/Testcases) | 10 (+ 1 HackerRank sample) |

## SỬA BÀI TẬP

- Show bài làm của các nhóm, nếu lên ý tưởng của bài toán, đưa ra code của nhóm.
- Một số bài làm tốt của các nhóm:
  + [HÀNG CÂY](./Homework/HANG_CAY/Review)
  + [TỔNG DÃY CON LỚN NHẤT](./Homework/TONG_DAY_CON_LON_NHAT/Review)
 
## THỐNG KÊ BUỔI HỌC
| **Tên nhóm** | **Số lần phát biểu** | Điểm cộng |
|:------------:|:----------:| --- |
| Nhóm 17      | 2         | 2 |
| Nhóm 3       | 1         | 1 |
| Nhóm 6       | 1         | 1 |
| Nhóm 11       | 1         | 1 |
| Nhóm 16       | 1         | 1 |
| Nhóm 9       | 1         | 1 |
| Nhóm 7       | 1         | 1 |
| Nhóm 4       | 1         | 1 |

## THỐNG KÊ CÁC NHÓM LÀM BÀI TẬP HACKERRANK

| Tên bài tập  | Các nhóm tham gia | Các nhóm passed |
|--------------|-------------------|-----------------|
| Tổng dãy con gần nhất | N05, N04, N10, N11, N7, N12, N15, N03, N14, N17, N09, N06, N13 | N1 N12, N04,  N15, N03, N14, N17 |
| Hàng cây  | N05, N04, N10. N11, N14, N09, N06, N15, N13, N03, N17, N7   |N05, N04, N10. N11, N14, N09, N06, N15, N13, N03, N17, N7 |

### Thống kê điểm các nhóm tham gia chủ đề Divide and Conquer

| Tên nhóm | Điểm hackerrank | Điểm tổng ( Điểm hackerranh + Điểm pháp biểu) |
| --- | --- | --- |
| N04 | 9 | 10 |
| N05 | 8 | 9 |
| N17 | 8 | 10 |
| N03 | 8 | 9 |
| N06 | 7 | 8 |
| N11 | 8 | 9 |
| N09 | 7 | 8 |
| N07 | 8 | 9 |
| N10 | 8 | 8 |
| N12 | 7 | 7 |
| N15 | 8 | 8 |
| N14 | 8 | 8 |
| N13 | 7 | 7 |

## ĐÁNH GIÁ VỀ CHỦ ĐỀ CỦA NHÓM
### Góp ý của giáo viên
**1. Ưu điểm:**
  - Nhóm trình bày rõ rành, slide đẹp, ít chữ.
  - Tuy là nhóm thuyết trình về các loai phương pháp thiết kế thuật toán đầu tiên nhưng nhìn chung là có chuẩn bị khá tốt.

**2. Nhược điểm:**
  - Cần bổ sung thêm phần nhận biết bài toán, vì chưa rõ ràng.
  - Phần thuyết trình của bạn chưa thực sự luôn cuốn người theo dõi.

### Nhóm tự đánh giá
  + Nhìn chung các bạn chú ý lắng nghe, tích cực tham gia đóng góp xây dụng bài và thỏa luận nhóm.
  + Nhóm xin chân thành cảm ơn thầy và các nhóm đã có những nhận xét đóng góp cho bài báo cáo được hoàn thiện và tốt hơn.
  + Qua đó, nhóm thấy rằng mình chuẩn bị chỉ mới tạm ổn. Vì vậy, nhóm đã rút ra nhiều kinh nghiệm cho những bài báo cáo sau này được tốt hơn, trau dồi kỹ năng thuyết trình cũng như thu hút người nghe nhiều hơn.

